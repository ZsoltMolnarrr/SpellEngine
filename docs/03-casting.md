...
- **`MELEE` delivery supports charging.** The output scaling covers both the weapon swing and the
  spell's impacts; `bonus` opts individual swing fields (momentum, reach, …) into scaling too.
  `melee_attacks` is the one `bonus` field ignored for melee — charge never adds swings. See
  [Delivery · Charged melee](05-delivery.md#charged-melee).
