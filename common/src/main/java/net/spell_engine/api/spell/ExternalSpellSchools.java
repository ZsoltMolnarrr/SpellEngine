...
public class ExternalSpellSchools {
    ...
    private static double offHandAttackDamage(LivingEntity entity) {
        ...
        var weaponDamage = entity.getAttributeBaseValue(EntityAttributes.ATTACK_DAMAGE)
                + AttributeModifierUtil.flatBonusFrom(offHandStack, EntityAttributes.ATTACK_DAMAGE);
        if (weaponDamage == 0) {
            return 0;
        }
        return weaponDamage * AttributeModifierUtil.multipliersOf(EntityAttributes.ATTACK_DAMAGE, entity);
    }
    ...
    private static RegistryEntry<EntityAttribute> rangedDamageAttribute() {
        if (Platform.util().isModLoaded("ranged_weapon_api")) {
            ...
            return EntityAttributes.ATTACK_DAMAGE; // Vanilla attack damage used as fallback
        }
    }
    ...
    public static void init() {
        ...
        EntityAttributes.ATTACK_DAMAGE.value().setTracked(true);
        PHYSICAL_MELEE.addSource(SpellSchool.Trait.POWER, SpellSchool.Apply.ADD, query -> {
            return query.entity().getAttributeValue(EntityAttributes.ATTACK_DAMAGE);
        });
        PHYSICAL_MELEE.addSource(SpellSchool.Trait.HASTE, SpellSchool.Apply.ADD, query -> {
            return AttributeModifierUtil.multipliersOf(EntityAttributes.ATTACK_SPEED, query.entity()) - 1.0;
        });
        ...
        DEFENSE.addSource(SpellSchool.Trait.POWER, SpellSchool.Apply.ADD, query -> {
            return query.entity().getAttributeValue(EntityAttributes.ARMOR);
        });
        SpellSchools.register(DEFENSE);

        HEALTH.addSource(SpellSchool.Trait.POWER, SpellSchool.Apply.ADD, query -> {
            return query.entity().getAttributeValue(EntityAttributes.MAX_HEALTH);
        });
