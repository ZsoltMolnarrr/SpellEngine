            ...
            if (impact.attribute != null && !impact.attribute.isEmpty()) {
                ...
                if (attributeEntry.isPresent()) {
                    ...
                    if (impact.attribute_from_target && attributeTarget instanceof LivingEntity livingEntity) {
                        ...
                        // registered. On 1.20.1 that is easy to hit — GENERIC_ATTACK_DAMAGE (used by
                        // Arsenal's weapon skills) is registered only by `HostileEntity`, so a villager,
                        // snow golem or animal caster would have crashed the server here.
