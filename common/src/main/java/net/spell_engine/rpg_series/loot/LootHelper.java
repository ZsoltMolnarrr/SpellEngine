...
public class LootHelper {
    ...
    public static void updateTagCache(LootConfig lootConfig) {
        ...
        var pools = new ArrayList<LootConfig.Pool>();
        pools.addAll(lootConfig.injectors.values());
        pools.addAll(lootConfig.regex_injectors.values());
        pools.addAll(lootConfig.fallbacks);
        for (var pool: pools) {
        ...
        for (var fallback: lootConfig.fallbacks) {
            tagsToCache.add(fallback.reference);
        }
    ...
    public static class FallbackReport {
        public String info = "Generated by the loot fallback injector, edits have no effect. Key: loot table id -> (config/category -> decision)";
    ...
    public static void configure(RegistryWrapper.WrapperLookup registries, Identifier lootTableId,
        ...
        if (pool != null) {
            poolSink.accept(buildPool(registries, pool, pool.rolls, isEntityLootTable, null));
            return;
        }
    ...
    private static void configureFallback(RegistryWrapper.WrapperLookup registries, String tableId,
        ...
        if (config.fallbacks.isEmpty()) { return; }
        var applicable = new ArrayList<LootConfig.Fallback>();
        for (var fallback: config.fallbacks) {
            if (!fallback.reference.isEmpty() && matchesId(tableId, fallback.table_pattern)) {
                applicable.add(fallback);
            }
        }
        ...
        for (var fallback: applicable) {
            var reference = fallback.reference;
            if (claimedCategories.contains(fallback.category)) { continue; }
            ...
            var effectiveRolls = fallback.rolls + fallback.extra_rolls * share;
            if (effectiveRolls <= 0) { continue; }
            claimedCategories.add(fallback.category);
            var mix = new EnchantMix(plainWeight, enchantedWeight, minLevel, maxLevel);
            poolSink.accept(buildPool(registries, fallback, effectiveRolls, isEntityLootTable, mix));
            ...
            pendingReport.injected
                    ...
                    .put(configName + "/" + fallback.category, String.format(Locale.ROOT,
                            "matched %s via %s, share %.2f, rolls %.2f, %s", matchedItem, reference, share, effectiveRolls, enchantInfo));
        }
    }
    ...
    private static LootPool buildPool(RegistryWrapper.WrapperLookup registries, LootConfig.Pool pool, float rolls,
                                      boolean isEntityLootTable, @Nullable EnchantMix mix) {
