...
public class SpellEngineEmiPlugin implements EmiPlugin {
    ...
    public void register(EmiRegistry registry) {
        ...
        // Spell books and scrolls are single items whose variants differ only in NBT.
        ...
        // - Scrolls are compared by full NBT, one entry per spell.
        registry.setDefaultComparison(SpellEngineItems.SPELL_BOOK.get(), Comparison.compareData(SpellEngineEmiPlugin::poolOf));
        registry.setDefaultComparison(SpellEngineItems.SCROLL.get(), Comparison.compareNbt());
        ...
        for (var tag : SpellBinding.availableSpellBookTags(world)) {
            ...
            for (var spellEntry : pool) {
                ...
                registry.addRecipe(new SpellBindingEmiRecipe(
                        ...
                        EmiStack.of(emptyBook),
                        consumablesFor(world, spellEntry),
     ...
     */
    private static EmiIngredient consumablesFor(World world, RegistryEntry<Spell> spellEntry) {
    ...
    private static String poolOf(EmiStack stack) {
        var container = SpellContainerHelper.containerFromItemStack(stack.getItemStack());
